...
public class BarFrame extends JFrame implements ChangeListener {
  ...
  */
  public BarFrame(DataModel dataModel) {
      ...
      public void paintIcon(Component c, Graphics g, int x, int y) {
        ...
        for (Double v : a) {
          ...
          Rectangle2D.Double rectangle =
              new Rectangle2D.Double(0, barHeight * i, barLength, barHeight);
          i++;
          g2.fill(rectangle);
        }
      }
    ...
    // TODO: add mouseclicklistener to each bar
    ...
    label.addMouseListener(new MouseAdapter() {
      public void mouseClicked(MouseEvent e) {
        ...
        for(int i = 0; i < a.size(); i++){
          if(y >= barIcon.getIconHeight() / a.size() * i && y <= barIcon.getIconHeight() / a.size() * (i + 1)){
            dataModel.update(i, x);
            break;
          }
        }
        // dataModel.update(1, 100);
      }
    });
